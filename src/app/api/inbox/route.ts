...
import { getInbox, markAllInboxRead, markInboxRead } from "@/lib/api";
...
export async function POST(request: Request) {
  ...
  try {
    const data = await markAllInboxRead(token);
    return NextResponse.json(data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to mark all read";
